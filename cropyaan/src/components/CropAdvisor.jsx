...
//             <span className="ca-crop-card__name">
//   {t(`crop.${crop.id}.name`)}
// </span>
//             {/* <span className="ca-crop-card__category">{crop.category}</span> */}
//             {/* <span className="ca-crop-card__category">
...
//           <p className="ca-crop-card__desc">
//   {t(`crop.${crop.id}.description`)}
// </p>
...
// <div className="ca-farmer-tip__text">{t(`crop.${crop.id}.tips`)}</div>          </div>
//         </div>
//       )}
//     </div>
//   );
...
function SeasonBadge({ season, t }) {
  ...
  return (
    <div className="ca-badge" style={{ background:c.bg, border:`1px solid ${c.border}` }}>
      <span style={{ fontSize:15 }}>{season.season.emoji}</span>
      <div>
        {/* Resolved via translation JSON instead of hardcoded JS strings */}
        <div className="ca-badge__label" style={{ color:c.text }}>
          {t(`season.${season.season.id}_label`)}
        </div>
        <div className="ca-badge__sub" style={{ color:c.text }}>
          {t(`season.${season.season.id}_months`)}
        </div>
      </div>
    </div>
  );
...
function SoilBadge({ soil, t }) {
  ...
  return (
    <div className="ca-badge" style={{ background:`${soil.primary.color}15`, border:`1px solid ${soil.primary.color}40` }}>
      <div style={{ width:10, height:10, borderRadius:"50%", background:soil.primary.color, flexShrink:0 }} />
      <div>
        {/* Resolved via translation JSON instead of hardcoded soilData label */}
        <div className="ca-badge__label" style={{ color:"var(--color-text)" }}>
          {t(`soil.${soil.primary.id}`)}
        </div>
        <div className="ca-badge__sub" style={{ color:"var(--color-text-muted)", opacity:1 }}>
          {t("crop_advisor.soil_type")}
        </div>
      </div>
    </div>
  );
    ...
    <div className="card ca-crop-card">
      ...
      >
        ...
        <div className="ca-crop-card__info">
          <div className="ca-crop-card__name-row">
            <span className="ca-crop-card__emoji">{crop.emoji}</span>
            <span className="ca-crop-card__name">{t(`crop.${crop.id}.name`)}</span>
            {/* category is now a lowercase key — resolved via t() */}
            <span className="ca-crop-card__category">{t(`crop_data.cat_${crop.category}`)}</span>
          </div>
          <p className="ca-crop-card__desc">{t(`crop.${crop.id}.description`)}</p>
          <ConfidenceBadge {...confidence} score={score} />
        </div>
      ...
      {expanded && (
        <div className="ca-crop-card__body">
          {/* Why it fits — strings already translated by recommendationEngine */}
          <div>
            <div className="ca-why-fits__title">{t("crop_advisor.why_fits")}</div>
            {reasons.map((r, i) => (
              <div key={i} className="ca-why-fits__reason">
          ...
          {/* Warnings — strings already translated by recommendationEngine */}
          {warnings.length > 0 && (
            <div className="ca-warnings">
              {warnings.map((w, i) => (
                <div key={i} className="ca-warnings__item">
          ...
          {/* Crop stats — waterRequirement and yieldPotential are now i18n keys */}
          <div className="ca-crop-stats">
            {[
              { label: t("crop_advisor.water_need"), value: t(`crop_data.water_${crop.waterRequirement}`) },
              { label: t("crop_advisor.duration"),   value: crop.growthDuration },
              { label: t("crop_advisor.yield"),      value: t(`crop_data.yield_${crop.yieldPotential}`) },
            ].map(({ label, value }) => (
              <div key={label} className="ca-crop-stat">
                <div className="ca-crop-stat__label">{label}</div>
                <div className="ca-crop-stat__value">{value}</div>
          ...
          <div className="ca-farmer-tip">
            <div className="ca-farmer-tip__label">{t("crop_advisor.farmer_tip")}</div>
            <div className="ca-farmer-tip__text">{t(`crop.${crop.id}.tips`)}</div>
          </div>
        </div>
      )}
    </div>
...
export default function CropAdvisor() {
  ...
  // Season note comes from translation JSON (crop_advisor.season_note_kharif etc.)
  const seasonNote = advisor.season?.season?.id
    ? t(`crop_advisor.season_note_${advisor.season.season.id}`)
    : "";
          ...
          {(step === STEP.CONDITIONS || step === STEP.INPUTS || step === STEP.RESULTS) && advisor.status === ADVISOR_STATUS.SUCCESS && (
            <div className="ca-stack">
              ...
              <div className="card rounded-2xl p-5">
                ...
                <div className="ca-badges-row">
                  {/* Pass t into both badges for translated labels */}
                  <SoilBadge soil={advisor.soil} t={t} />
                  <SeasonBadge season={advisor.season} t={t} />
                </div>
                {seasonNote && (
                  <div className="ca-season-note">
                    <Info size={12} style={{ flexShrink:0, marginTop:1 }} />
              ...
              {/* Advisory warning — strings already translated by useCropAdvisor */}
              {advisor.advisory && (
                <div className="ca-advisory">
                  <AlertTriangle size={14} style={{ flexShrink:0, marginTop:1 }} />
                  <span>{advisor.advisory.join(" ")}</span>
          ...
          {step === STEP.RESULTS && advisor.recommendations.length > 0 && (
            <div className="ca-stack">
              <div className="card rounded-2xl p-5">
                ...
                <p className="ca-results-meta">
                  {t("crop_advisor.results_based")} {t(`season.${advisor.season?.season?.id}_label`)} {t("crop_advisor.results_season")} · {t(`soil.${advisor.soil?.primary?.id}`)} · {farmInputs?.water} · {farmInputs?.land}
                </p>
                <div className="ca-crop-list">
                  {advisor.recommendations.map((result, i) => (
                    <CropCard key={result.crop.id} result={result} rank={i} t={t} />
              ...
              {advisor.soil?.all?.length > 1 && (
                <div className="ca-other-soils">
                  ...
                  <span>
                    <strong>{t("crop_advisor.other_soils")}: </strong>
                    {advisor.soil.all.slice(1).map(s => t(`soil.${s.id}`)).join(", ")}
                  </span>
                </div>
              )}
